...
val bofo15Schema = StructType(bofo15fields.split(",").toList.map(x => schemaMapper(x, bofo15Keys.split(",").toList)))
...
val bofo12Schema = StructType(bofo12fields.split(",").toList.map(x => schemaMapper(x, bofo12Keys.split(",").toList)))
...
val bofo20Schema = StructType(bofo20fields.split(",").toList.map(x => schemaMapper(x, bofo20Keys.split(",").toList)))
...
val bofo08Schema = StructType(bofo08fields.split(",").toList.map(x => schemaMapper(x, bofo08Keys.split(",").toList)))
...
val BCNFMILSPSchema = StructType(BCNFMILSPfields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//ZPCUSTOMER -- This will schema change - there are definetely more than 3 columns in any CUSTOMER MasterData table
val ZPCUSTOMERfields = "CUSTOMER,OBJVERS,CUSTGRP3"
val ZPCUSTOMERUsedForBofo30Seq = ""
val ZPCUSTOMERSchema = StructType(ZPCUSTOMERfields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//BCNFSTAGP -- No Schema given in BOFO30 FS
val BCNFSTAGPFields = "BCNFSTAGP,OBJVERS,CHANGED,BCNFCHR01,DOC_TYPE"
val BCNFSTAGPUsedForBofo30Seq = "BPIIN_13,DOC_TYPE"
val BCNFSTAGPSchema = StructType(BCNFSTAGPFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//BCNFGIPG -- No Schema given in BOFO30 FS
val BCNFGIPGFields = "BCNFGIPG,OBJVER,CHANGED,BPRICODE,BCNFCHR01"
val BCNFGIPGUsedForBofo30Seq = ""
val BCNFGIPGSchema = StructType(BCNFGIPGFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//BCNFCASRP -- No Schema given in BOFO30 FS
val BCNFCASRPFields = "BCNFCASRP,OBJVERS,CHANGED,BORGDNO01,BORGDNO11,BCNFIPG,BCNFCHR01"
val BCNFCASRPUsedForBofo30Seq = ""
val BCNFCASRPSchema = StructType(BCNFCASRPFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//BCNFNMCS -- No Schema given in BOFO30 FS
val BCNFNMCSFields = "BCNFNMCS,OBJVERS,CHANGED,BCNFIPG,BNS_RDD,BCNFCHR01"
val BCNFNMCSUsedForBofo30Seq = ""
val BCNFNMCSSchema = StructType(BCNFNMCSFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))
...
val BCNFACTSPSchema = StructType(BCNFACTSPFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//BCNFSHPQY
val BCNFSHPQYFields = "BCNFSHPQY,OBJVERS,CHANGED,BORD_TYPE,ITEM_CATEG,BCONFDFLG,BCNFCHR01"
val BCNFSHPQYUsedForBofo30Seq = ""
val BCNFSHPQYSchema = StructType(BCNFSHPQYFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//BICPPCTR
val BICPPCTRFields = "BICPPCTR,OBJVERS,CHANGED,BLOPRCTR,BHIPRCTR,BMM_PLT"
val BICPPCTRUsedForBofo30Seq = ""
val BICPPCTRSchema = StructType(BICPPCTRFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))
...
val MaterialSchema = StructType(MaterialFields.split(",").toList.map(x => schemaMapper(x, "".split(",").toList)))

//bofo30
val bofo30Keys = "DOC_NUMBER,S_ORD_ITEM"
val bofo30fields = "DOC_NUMBER,S_ORD_ITEM,DOC_TYPW,BPONBR,BMIPRNBR,BMGMT_CD,BMEDST_CD,BINVCNT,PSTNG_DATE,CURRENCY,BORDER_DT,CML_CD_QTY,MIN_DL_QTY,BASE_UOM,LOWR_BND,DOC_CURRCY,BPURPRIC,CONF_QTY,NET_VALUE,GIS_QTY,DLVQEYCR,DLVQEYSC,DLVQLECR,BCONVERT,DLVQLESC,SUBTOTAL_3,SUBTOTAL_4,SUBTOTAL_5,SUBTOTAL_6,SUBTOTAL_2,SUBTOTAL_1,QCOASREQ,DLV_QTY,BGR_DATE,BCUSTPODT,BNS_RDD,BORD_TYPE,BBSTNK,BORGDNUM,BPRI_CD,BMPC,LOC_CURRCY,UPPR_BND,SALES_UNIT,BSHIP_QTY,BTGT_SHIP,BTGTSHIP,BFUND_CD,BLOA,BMIPR,BMODDT,BPROJ_CD,BPURPOSE,BTYPEASST,MATL_GRP_1,MATL_GRP_2,MATL_GRP_3,MATL_GRP_5,PAYER,PLANT,PRICE_DATE,PROD_HIER,PROFIT_CTR,REFER_DOC,REFER_ITM,REJECTN_ST,ROUTE,SALES_GRP,SALES_OFF,SHIP_POINT,SHIP_STCK,SHIP_TO,STAT_DATE,STOR_LOC,STS_BILL,STS_ITM,STS_PRC,STS_DEL,WBS_ELEMT,BADV_CD,BBILL_AD,BBIL_ST,BCAGE_CD,BCLSSA,BCNLDDT,BCNTRLNBR,BCONDCTRL,BCONT_LN,BDEMAND,BDISTRIB,BDOCIDCD,BDODAAC,BEXC_INFO,BFMS_PRG,BPRIUSRID,BRETNTQTY,BRICFROM,BSHIPNBR,BSIG_CD,BSPLPGMRQ,BSUPPADR,SALESORG,BOWNRSHP,BZZSED,MATL_GRP4,BMFR_NAME,BMFRPRTNO,BKDMAT,BPROJ_NUM,STORNO,BCOUNTER,BMFR_CAGE,BITMCATGP,BMATRCVDT,BSUFX_CD,BSUPP_ST,DLV_STSO,DLV_STS,LST_A_GD,LW_GISTS,BMATL_BLK,CML_CF_QTY,REQDEL_QTY,BFMSCSCD,BIPG,DSDEL_DATE,GR_QTY,REQ_QTY,REASON_REJ,SOLD_TO,BTKILLQTY,BTKILLUOM,BTORDQTY,BTVRSTCD,BBO_TYPE,BSHIPTYP,BDISCP_CD,BMRA_QTY,BDISCP_CD,BMRA_QTY,BDISCPQTY,BATCH,BILL_DATE,BILL_TYPE,BILLTOPRTY,BND_IND,CH_ON,CML_OR_QTY,CO_AREA,COMP_CODE,CREA_TIME,CREATEDBY,CREATEDON,CUST_GROUP,CUST_GRP1,CUST_GRP2,CUST_GRP3,CUST_GRP4,CUST_GRP5,DEL_BLOCK,DISTR_CHAN,DIVISION,DOC_CATEG,DOC_CATEGR,FISCVARNT,FORWAGENT,ITEM_CATEG,MATERIAL,MATL_GROUP,ST_UP_DTE,B_CAS,B_CRM,B_SST,BCONDITN,BDMDPLANT,NET_PRICE,LOAD_DATE,BSTAGRP,COND_PR_UN,BACT_SHIP,BCNDQTY,BDMDQTY,BAVAILIND,CALMONTH,ITEM_DEL,REQU_QTY,BZZRIC,BSHIP_MON,BPIIN_13,ACT_GI_DTE,PO_UNIT,BMILSVC_1,BMILSVC_2,BMILSVC_3,BAUDAT,PCONF_QTY,BNMCS_IND,BCASREP,CONF_DATE,DOCTYPE,BACTSHIP,BCAGECDPN,BIDNLF,BCREATEDT,DOC_DATE,B_YOBLOCK,B_ZTBLOCK,BMFRPN,BOFBNSRDD,BOFITMCAT,BPLT,SCL_DELDAT,B_ALT,BOFPRICD,BMOQ,BRIC_TO,BHDRCRTDT,CONF_TYPE,BPARTNER,BZZSSC,BZZDMDDN,BZZDMDSC,BZZODNPST,BZZJOKO,BZZUSECD,BZZCHRNCD,BZZPRIREF,BZZINSPCD,BZZPICKLS,BZZMATAQC,BZZRICSOS,BZZUTILCD,BEARCDTE,BGFM_CLIN,BGFM_CONT,BGFM_CALL,BMIPR_NBR,BGFM_MDN,BEMRADTE,BCSTACCDT,BCSTACCQY,BTPDDATE,BTPDQTY,BEMRAQTY,BTRNTYPCD,BBASISACK,BSHPMANUM,BSDAQUAN,BSDADATE"
val bofo30Schema = StructType(bofo30fields.split(",").toList.map(x => schemaMapper(x, bofo30Keys.split(",").toList)))
def createTestDataFrame: (Int, StructType) => DataFrame = (numOfRows: Int, schema: StructType) => {
   spark.createDataFrame( 
                          (for (i <- 1 to numOfRows)  yield 
                            Row.fromSeq((for (a <- schema)  yield a match {
                            case a if a.dataType == DateType => /*if (scala.util.Random.nextInt() % i < 2) null else */ new java.sql.Date(new SimpleDateFormat("dd-MM-yyyy").parse(LocalDate.ofEpochDay( new Random().nextInt(150000)).toString).getTime())
                            case a if (a.dataType == StringType && a.name == "DOC_CATEG") => "ABCDEIK".charAt(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString()
                            case a if (a.dataType == StringType && a.name == "OBJVERS") => "ABCD".charAt(java.lang.Math.abs(scala.util.Random.nextInt() % 4)).toString()
                            case a if (a.dataType == StringType && a.name == "DOC_NUMBER") => val a:Array[String] = Array("100000","200000","300000","400000", "500000", "60000000", "700000", "800000"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 7)).toString()
                            case a if (a.dataType == StringType && a.name == "BDODAAC") => val s:String = "ABCDE"; s.charAt(java.lang.Math.abs(scala.util.Random.nextInt() % 4)).toString() + s.charAt(java.lang.Math.abs(scala.util.Random.nextInt() % 4)).toString() + s.charAt(java.lang.Math.abs(scala.util.Random.nextInt() % 4)).toString()
                            case a if (a.dataType == StringType && a.name == "CUST_GRP3") => val a:Array[String] = Array("EPMD","NWA","NBN","MD", "BBD", "B2M", "WUTANG"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 5)).toString() 
                            case a if (a.dataType == StringType && a.name == "SOLD_TO") => val a:Array[String] = Array("CUST1","CUST2","CUST3","CUST4", "CUST5", "CUST6"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 5)).toString()
                            case a if (a.dataType == StringType && a.name == "S_ORD_ITEM") => val a:Array[String] = Array("0010","0020","0040","0030", "0050"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 4)).toString() 
                            case a if (a.dataType == StringType && a.name == "DOC_TYPE") => val a:Array[String] = Array("BLUE","AQUA","LAVA","PINK", "KIWI", "ZTVC", "ZTVR"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString() 
                             case a if (a.dataType == StringType && a.name == "MATERIAL") => val a:Array[String] = Array("STEEL","WOOD","PLASTIC","ALUMINUM", "STYROFOAM", "GLASS", "RUBBER"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString()
                             case a if (a.dataType == StringType && a.name == "CREATEDON") => val a:Array[String] = Array("20170101","20151101","20161201","20161002", "20141010", "20150801", "20081111"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString()
                             case a if (a.dataType == StringType && a.name == "BORD_TYPE") => val a:Array[String] = Array("A","B","C","D", "E", "F", "*"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString()
                             case a if (a.dataType == StringType && a.name == "ITEM_CATEG") => val a:Array[String] = Array("ZRTN","ZTVC","ZSTO","ZKIT", "ZCT1", "ZBDC", "ZDRT", "ZPH"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 7)).toString()
                             case a if (a.dataType == StringType && a.name == "CONF_DATE") => val a:Array[String] = Array("20170101","20151101","20161201","", "20141010", "20150801", "20081111"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString()
                             case a if (a.dataType == StringType && a.name == "BOFBNSRDD") => val a:Array[String] = Array("20170201","20150201","20160201","", "20140210", "20150801", "20081111"); a(java.lang.Math.abs(scala.util.Random.nextInt() % 6)).toString()
                            case a if (a.dataType == StringType && a.name == "BFMS_PRG") => val s:String = "ABCDE"; s.charAt(java.lang.Math.abs(scala.util.Random.nextInt() % 4)).toString()
                            case a if a.dataType == StringType => scala.util.Random.alphanumeric.take(10).mkString
                            case a => new java.math.BigDecimal(scala.util.Random.nextInt(), new java.math.MathContext(3))
                            }).toSeq)
                          ).toList.asJava
                        , schema
                        )
...
//val BCNFMILS2PDF = createTestDataFrame(rows, BCNFMILSPSchema) 
//val ZPCUSTOMERDF = createTestDataFrame(rows, ZPCUSTOMERSchema) 
val BCNFSTAGP = createTestDataFrame(rows, BCNFSTAGPSchema) 
val BICPPCTR = createTestDataFrame(rows, BICPPCTRSchema) 
val BCNFGIPGDF = createTestDataFrame(rows, BCNFGIPGSchema) 
val BCNFCASRPDF = createTestDataFrame(rows, BCNFCASRPSchema) 
val BCNFNMCSDF = createTestDataFrame(rows, BCNFNMCSSchema) 
val BCNFMILSPDF = Seq(("0010", "A", "A", "A", "B", "A", "*", "Y","A","A", "AAD", "DDA", "AAAAAAAAAA"),
                      ("0020", "B", "*", "*", "*", "*", "*", "Y","A","B", "DAA", "ACC", "BBBBBBBBBB"),
                      ("0050", "A", "*", "B", "*", "A", "*", "Y","A","D", "ADA", "CCA", "CCCCCCCCCC"),
                      ("0060", "A", "C", "B", "C", "C", "*", "Y","A","B", "LLL", "ACA", "DDDDDDDDDD"),
                      ("0030", "B", "C", "*", "A", "B", "*", "Y","A","B", "KKK", "FFF", "AAAAAAAAAA"),
                      ("0050", "C", "I", "C", "A", "B", "Y", "Y","A","B", "ZAA", "GGG", "AAAAABBBBB"),
                      ("0040", "A", "B", "C", "C", "*", "Y", "Y","A","D", "AXA", "EEE", "AAAAACCCCC"),
                      ("0060", "A", "B", "C", "A", "*", "*", "Y","A","D", "AXA", "EEE", "AAAAACCCCC"),
                      ("0020", "I", "C", "A", "A", "*", "*", "Y","D","D", "AAV", "DDD", "ACCCAAADDD")).toDF("BCNFMILSP","OBJVERS","CHANGED","BSERVICE1","BSERVICE2","BSERVICE3","BFMS_IND","BMILGP1FG","BMILGP2FG","BMILGP3FG","BMILGP1RS","BMILGP2RS","BMILGP3RS")
                      
val ZPCUSTOMERDF = Seq(("EJnb9IccRI", "A", "AAA"),
                      ("0et2E5sTBM", "B", "ABB") ,
                      ("wTxdgxE1EW", "A", "ABB") ,
                      ("WfeqByH3uZ", "A", "ABB") ,
                      ("6yg86E4XZ6", "B", "ABB") ,
                      ...
                      ("6yg86E4XZq", "A", "ABB") ,
                      ("6yg86E4XZx", "A", "ABB") ,
                      ("6yg86E4XZa", "I", "C") ).toDF("CUSTOMER","OBJVERS","CUSTGRP3")
  
val BCNFACTSP  = spark.read.format("csv")
                     .option("header", "false")
                     .option("delimiter", ",")
                     .schema(BCNFACTSPSchema)
                     .load("/data/bcnfactsp/BCNFACTSP.csv")
                     
val BCNFSHPQY  = spark.read.format("csv")
                     .option("header", "false")
                     .option("delimiter", ",")
...
val MaterialDF = createTestDataFrame(rows, MaterialSchema) 
   ...
   val DistinctMaterialDF = MaterialDF.filter($"OBJVERS" === lit("A"))
                                        ...
                                        .select(MaterialDF.col("BACQADVCD"),$"BADMLDTME",$"BPRODLDTM",$"PROFIT_CTR", $"OBJVERS", $"MATERIAL").dropDuplicates
...
//Start Routine
   ...
   val DistinctMaterialDF = MaterialDF.filter($"OBJVERS" === lit("A"))
    ...
    val distinctBTypeItemCatConfDateDF = bof20DF.select($"BORD_TYPE", when(length(col("CONF_DATE")) > lit(0), lit("G")).otherwise(lit("")).alias("CONF_DATE_FLAG"),$"ITEM_CATEG").distinct
    val distinctReasonActgiItemCategConfDateDF = bof20DF.select(when(length(col("REASON_REJ")) < lit(1), lit("<>")).otherwise(col("REASON_REJ")).alias("REASON_REJ_CHK"),
    ...
    val byBTypeItemCatConfDateOrderByBcnfshpqy = Window.partitionBy($"BORD_TYPE",$"ITEM_CATEG",$"CONF_DATE_FLAG").orderBy($"BCNFSHPQY".asc)
    val byItemCatReasonActgiConfDateOrderByBcnfactsp = Window.partitionBy($"ITEM_CATEG",$"REASON_REJ_CHK",$"ACT_GI_FLAG",$"CONF_DATE_FLAG").orderBy($"BCNFACTSP".asc)
    ...
    val ingestAfterBofo20DF = ingestBeforeBofo20DF.alias("bof20DF").filter(length(col("bof20DF.DOC_CATEG")) > lit(0))
                                                  .withColumn("CONF_DATE_FLAG", when(length(col("CONF_DATE")) > lit(0), lit("G")).otherwise(lit(""))).alias("ingestBeforeBofo20ConfDateFlagDF")
                                                  .join(shipQtyfromBcnfshpqy.alias("shipQtyfromBcnfshpqy"), col("shipQtyfromBcnfshpqy.ITEM_CATEG") === col("ingestBeforeBofo20ConfDateFlagDF.ITEM_CATEG") &&
                                                  ...
                                                  .withColumn("BSHIP_QTY", when(col("BCNFCHR01") === lit("I"), col("GIS_QTY"))
                                                                          .otherwise(when(col("BCNFCHR01") === lit("G"), col("GR_QTY"))
                                                                                    .otherwise(when(col("BCNFCHR01") === lit("C"), col("CONF_QTY"))
                                                                                              .otherwise(when(col("BCNFCHR01") === lit("D"), col("DLV_QTY"))
                                                  ...
                                                  .drop(col("shipQtyfromBcnfshpqy.ITEM_CATEG"))     
                                                  .drop(col("shipQtyfromBcnfshpqy.BORD_TYPE"))  
                                                  .drop(col("shipQtyfromBcnfshpqy.CONF_DATE_FLAG"))  
                                                  .drop(col("BCNFCHR01"))
                                                  .withColumn("REASON_REJ_CHK", when(length(col("REASON_REJ")) < lit(1), lit("<>")).otherwise(col("REASON_REJ")))
                                                  .withColumn("ACT_GI_FLAG", when(length(col("ACT_GI_DTE")) < lit(1), lit("N")).otherwise(lit("Y")))
                                                  .withColumn("CONF_DATE_FLAG", when(length(col("CONF_DATE")) < lit(1), lit("N")).otherwise(lit("Y"))).as("ingestBeforeBofo20AllJoinFlagsDF")
                                                  .join(actShipfromBcnfactsp.alias("actShipfromBcnfactsp"), col("actShipfromBcnfactsp.ITEM_CATEG") === col("ingestBeforeBofo20AllJoinFlagsDF.ITEM_CATEG") &&
                                                  ...
                                                  .withColumn("BACT_SHIP", when(col("BCNFCHR01") === lit("L"), col("LOAD_DATE"))
                                                                          .otherwise(when(col("BCNFCHR01") === lit("G"), col("BGR_DATE"))
                                                                                    .otherwise(when(col("BCNFCHR01") === lit("C"), col("CONF_DATE"))
                                                                                              .otherwise(when(col("BCNFCHR01") === lit("P"), col("BCUSTPODT"))
                                                                                                        .otherwise(when(col("BCNFCHR01") === lit("M"), col("BMATRCVDT"))
                                                                                                                  .otherwise(when(col("BCNFCHR01") === lit("O") &&
                                                                                                                                length(col("BCNFCHR01")) < lit(1), julianDateParseUDF(col("BMATRCVDT").substr(14,4)))
                                                                                                                            .otherwise(when(col("BCNFCHR01") === lit("B"), lit(""))
                                                                                                                                      .otherwise(lit(""))
                                                                                                                            )
                                                                                                                  )
                                                                                                        )
                                                  ...
                                                  .join(bcnfstagpDF.alias("bcnfstagpDF"), bcnfstagpDF.col("DOC_TYPE") === ingestBeforeBofo20DF.col("DOC_TYPE"), "left") 
                                                  .drop(col("bcnfstagpDF.DOC_TYPE"))
                                                  .join(bicppctrDF.alias("bicppctrDF"), (ingestBeforeBofo20DF.col("PROFIT_CTR") >= bicppctrDF.col("BLOPRCTR")) && 
                                                       (ingestBeforeBofo20DF.col("PROFIT_CTR") <= bicppctrDF.col("BHIPRCTR")), "left") 
                                                  .withColumn("GV_CREATED_ON", when(col("bcnfstagpDF.DOC_TYPE").isNull, col("BORDER_DT")).otherwise(col("CREATEDON")))
                                                  .withColumn("BTGT_SHIP", when(col("BOFITMCAT") isin (matlItemCatGrpList1), lit(""))
                                                                          .otherwise(when((col("ITEM_CATEG") isin (rSalesDocItemCat1Line)) && 
                                                                                          (length(col("GV_CREATED_ON")) > lit(0)), addDays(col("GV_CREATED_ON"), lit(365)))
                                                                                    .otherwise(when((col("DOC_TYPE") isin (rSalesDocType1Line)) && 
                                                                                                    (length(col("GV_CREATED_ON")) > lit(0)), addDays(col("GV_CREATED_ON"), lit(15)))
                                                                                              .otherwise(when((col("DOC_TYPE") isin (rSalesDocType2Line)) && (col("PROFIT_CTR").substr(3,2) === lit("21")) &&
                                                                                                             (length(col("GV_CREATED_ON")) > lit(0)), addDays(col("GV_CREATED_ON"), lit(6)))
                                                                                                        .otherwise(when((col("DOC_TYPE") isin (rSalesDocType2Line)) && 
                                                                                                                    (length(col("BZZESD")) > lit(0)) && 
                                                                                                                    (length(col("GV_CREATED_ON")) > lit(0)) && 
                                                                                                                    (col("BZZESD") >= addDays(col("GV_CREATED_ON"), lit(2))),col("BZZESD"))
                                                                                                                  .otherwise(when((col("DOC_TYPE") isin (rSalesDocType2Line)) && 
                                                                                                                                 (length(col("BZZESD")) > lit(0)) && 
                                                                                                                                 (length(col("GV_CREATED_ON")) > lit(0)) && 
                                                                                                                            ...
                                                                                                                            .otherwise(when((col("DOC_TYPE") isin (rSalesDocType2Line)) && 
                                                                                                                                            ...
                                                                                                                                            (length(col("GV_CREATED_ON")) > lit(0)) && 
                                                                                                                                    (col("DSDEL_DATE") >= addDays(col("GV_CREATED_ON"), lit(2))),col("DSDEL_DATE"))
                                                                                                                                      .otherwise(when((col("DOC_TYPE") isin (rSalesDocType2Line)) && 
                                                                                                                                            (length(col("DSDEL_DATE")) > lit(0)) && 
                                                                                                                                            (length(col("GV_CREATED_ON")) > lit(0)) && 
                                                                                                                                            (col("DSDEL_DATE") < addDays(col("GV_CREATED_ON"),
                                                                                                                                                lit(2))),addDays(col("GV_CREATED_ON"), lit(2))
                                                                                                                                                 .otherwise(when((col("DOC_TYPE") isin (rSalesDocType2Line)) &&
                                                                                                                                                            (length(col("GV_CREATED_ON")) > lit(0))
                                                                                                                                                                ,addDays(col("GV_CREATED_ON"), lit(15)))
                                                                                                                                                    .otherwise(when((col("DOC_TYPE") isin (rSalesDocType3Line)),
                                                                                                                                                                col("DSDEL_DATE"))
                                                                                                                                                       .otherwise(when((col("DOC_TYPE") isin (rSalesDocType4Line)) &&
                                                                                                                                                                (length(col("DSDEL_DATE"))  > lit(0)), 
                                                                                                                                                                addDays(col("DSDEL_DATE"),lit(5)))
                                                                                                                                                       .otherwise(when((col("DOC_TYPE") isin (rSalesDocType5Line)) &&
                                                                                                                                                            (col("ITEM_CATEG") isin (rSalesDocItemCat2Line)) &&
                                                                                                                                                                (length(col("GV_CREATED_ON")) > lit(0)), 
                                                                                                                                                                addDays(col("GV_CREATED_ON"),lit(197)))
                                                                                                                                                       .otherwise(when((col("CUST_GRP3") === lit("FMS")) &&
                                                                                                                                                ...
                                                                                                                                                date_format(
                                                                                                                                                 date_add(
                                                                                                                                                  from_unixtime(
                                                                                                                                                    unix_timestamp(col("GV_CREATED_ON"),
                                                                                                                                                        "yyyyMMdd"),
                                                                                                                                                    "yyyy-MM-dd"), 
                                                                                                                                                 col("BADMLDTME").plus() col("BPRODLDTM")), "yyyyMMdd"))
                                                                                                                                                        .otherwise(when((col("PROFIT_CTR") >= col("BLOPRCTR")) &&
                                                                                                                                                 ...
                                                                                                                                                 date_add(
                                                                                                                                                  from_unixtime(
                                                                                                                                                    unix_timestamp(col("GV_CREATED_ON"),
                                                                                                                                                        "yyyyMMdd"),
                                                                                                                                                    "yyyy-MM-dd"), 
                                                                                                                                                 col("BMM_PLT")), "yyyyMMdd"))
                                                                                                                                                  .otherwise(when((col("BFMS_PRG") isin (rFmsProgCdLine)) &&
                                                                                                                                                                  (col("CUST_GRP3") === lit("FMS")),
                                                                                                                                                                  addDays(col("GV_CREATED_ON"), lit(180)))
                                                                                                                                        .otherwise(when(col("BOFBNSRDD").substr(2,2) isin (isNddExtendedList) &&
                                                                                                                                                         (col("BOFBNSRDD").substr(1,1) === lit("S")),
                                                                                                                                                         getLastDoMUdf(col("BOFBNSRDD")))
                                                                                                                                    .otherwise(when((col("BORD_TYPE") === lit("C")) &&
                                                                                                                                                 ...
                                                                                                                                                 date_add(
                                                                                                                                                  from_unixtime(
                                                                                                                                                    unix_timestamp(col("GV_CREATED_ON"),
                                                                                                                                                        "yyyyMMdd"),
                                                                                                                                                    "yyyy-MM-dd"), 
                                                                                                                                                 col("BADMLDTME") + col("BPRODLDTM")), "yyyyMMdd")))   
                                                                                                                                    .otherwise(when(((col("BORD_TYPE") === lit("C")) &&
                                                                                                                                                     (col("BACQADVCD") isin (rAac2Line))  &&
                                                                                                                                                       ((col("BADMLDTME") <= lit(0)) ||
                                                                                                                                                            (col("BPRODLDTM") <= lit(0))),
                                                                                                                                            addDays(col("GV_CREATED_ON"), lit(180)))) 
                                                                                                                                    .otherwise(when((col("BOFPRICD") isin (rHighPriorityLine)),
                                                                                                                                            addDays(col("GV_CREATED_ON"), lit(2)))                                                              .otherwise(when((col("BOFPRICD") isin (rLowPriorityLine)) && 
                                                                                                                                            ((col("BOFBNSRDD").substr(1,1) isin (rExpNsRdd2Line)) ||
                                                                                                                                                        (col("BOFBNSRDD") isin (rExpNsRdd1Line))) 
                                                                                                                                            addDays(col("GV_CREATED_ON"), lit(2))) 
                                                                                                                            .otherwise(when((col("BOFPRICD") isin (rLowPriorityLine)) && 
                                                                                                                                            (!(col("BOFBNSRDD").substr(1,1) isin (rExpNsRdd2Line)) &&
                                                                                                                                                        (!(col("BOFBNSRDD") isin (rExpNsRdd1Line)))), 
                                                                                                                                            addDays(col("GV_CREATED_ON"), lit(4))) )))
                                                                                                                                            )
                                                                                                                                        )
                                                                                                                                    )
                                                                                                                                )
...
/** Start BOFO15 **/
val ingestAfterbofo15DF = bof15DF.filter(col("DOC_CATEG") =!= lit("C"))
                                 .filter(col("DOC_CATEG") =!= lit("I"))
                                 /* Does The Record Mode column come in as " " like in FS or come in as null */
                                 /* .withColumn("NEW_RECORD_MODE", when(col("REC_DELETD") <=> lit("X") && col("RECORDMODE") <=> lit(""), lit("A")).otherwise(col("RECORDMODE"))).drop("RECORDMODE").withColumnRenamed("NEW_RECORD_MODE","RECORDMODE") */
...
val byCUST_GRP3andBDODAACorderedByBCNFMILSP = Window.partitionBy($"CUST_GRP3", $"BDODAAC").orderBy($"BCNFMILSP".desc)
val byCUST_GRP3andBDODAACandSOLD_TOorderedByBCNFMILSP = Window.partitionBy($"CUST_GRP3", $"SOLD_TO",$"BORGDNUM").orderBy($"BCNFMILSP".desc)    


/*Subsets of rule criteria columns for MILSVC transformations */    
val distinctCUST_GRP3andBDODAACDF = bof12DF.select($"CUST_GRP3",$"BDODAAC").distinct
val distinctCUST_GRP3andBDODAACandSOLD_TODF = bof12DF.select($"CUST_GRP3",$"BORGDNUM",$"SOLD_TO").distinct

/*Subsets of rule criteria columns for BMILSVC_1 transformation with BMILSVC_1 */
val distinctCUST_GRP3andBDODAACwithBMILSVC_1DF = selectBmilsvc1(distinctCUST_GRP3andBDODAACDF, BCNFMILSPDF, byCUST_GRP3andBDODAACorderedByBCNFMILSP).get 
...
val bof12WithBMILSVC_1 = ingestBeforeBofo12DF.join(distinctCUST_GRP3andBDODAACwithBMILSVC_1DF, distinctCUST_GRP3andBDODAACwithBMILSVC_1DF.col("CUST_GRP3") === bof12DF.col("CUST_GRP3") &&
...
val distinctCUST_GRP3andBDODAACwithBMILSVC_2DF = selectBmilsvc2(distinctCUST_GRP3andBDODAACDF, BCNFMILSPDF, byCUST_GRP3andBDODAACorderedByBCNFMILSP).get
distinctCUST_GRP3andBDODAACwithBMILSVC_2DF.show()

/*After the BMILSVC_2 transformation joined back to bofo12 */
val bof12WithBMILSVC_2 = bof12WithBMILSVC_1.join(
...
val bcnfactspDF  = spark.read.format("csv")
                     .option("header", "false")
                     .option("delimiter", ",")
                     .schema(BCNFACTSPSchema)
                     .load("/data/bcnfactsp/BCNFACTSP.csv")
...
val bcnfshpqyDF  = spark.read.format("csv")
                     .option("header", "false")
                     .option("delimiter", ",")
                     .schema(BCNFSHPQYSchema)
                     .load("/data/bcnfshpqy/BCNFSHPQY.csv")
...
val ingestAfterBofo20DF = ingestBeforeBofo20DF.filter(col("DOC_CATEG") =!= lit("C"))
                                              .filter(col("DOC_CATEG") =!= lit("I"))
                                              //find the current massageAndErrorCheckDF method
                                              //Need verification on PSTINGDATE column
                                              .withColumn("BSHIP_QTY", lit("00000000"))
                                              .withColumn("PSTNG_DATE", dateParseUdf(col("BGR_DATE"))).drop("BGR_DATE")
...
*/
    ...
    bofo20DF.withColumn("LAG_DATE", lagDateUDF(col("BTGT_SHIP")))
        .withColumn("ZQUANTITY", when(col("GR_QTY") > lit(0),col("GR_QTY")).otherwise(col("BSHIP_QTY")))
        .withColumn("BAVAILIND", 
            when(col("DOC_TYPE") === lit("ZTVC") || col("DOC_TYPE") === lit("ZCAT"), 
                (when(col("ITEM_CATEG") === lit("ZCAT"), lit("Y"))
                    .otherwise(when(col("REASON_REJ") isin (reasonList), lit("R"))
                        .otherwise(when(col("CONF_QTY") === col("BTKILLQTY") && col("REASON_REJ") === lit("AK"), lit("R"))
                            .otherwise(when(col("BACT_SHIP") > col("BTGT_SHIP"), lit("N"))
...
/*Assign the bofo08 columns from the bofo12 to bofo30 Spec in the bofo08  */
import java.util.Calendar
import java.util.GregorianCalendar
import java.text.DateFormat
import java.text.Format
...
val ZPCUSTOMERDF = Seq(("EJnb9IccRI", "A", "AAA"),
    ...
    def dateParse = udf((rawDate: String) => {
                                          new java.sql.Date(new java.text.SimpleDateFormat("yyyyMMdd").parse(rawDate).getTime())
        } )
...
val dateTestDF = Seq(("20161414"),("99999999"),("00000000"),("20110101")).toDF("dateTest")

dateTestDF.select($"dateTest", expr("DATE_FORMAT(CAST(UNIX_TIMESTAMP(dateTest,'yyyyMMdd') AS TIMESTAMP), 'yyyy-MM-dd') as dateTest_1"), dateParse($"dateTest"), dateParseUdf($"dateTest")).show()
