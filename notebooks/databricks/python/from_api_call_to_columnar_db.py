...
"""
##### Subject
This pipeline pulls from Subject API (v12 https://apihelp.Subject.io/help/objects) and updates 3 ColumnStoreDB tables in two stages.

This pipeline is an almost exact rewrite (from Ruby to Python) of an existing ETL pipeline as part of a migration project and consequently all of its warts. This was (unfortunately) done to maintain consistency with historical data, specifically subjects that are currently running.

##### _Stage 1_

Produces __subjects_df__

1. Query existing Subject_subjects table for most recently modified subject
2. For any subject in the API that is modified more recently, add a new entry to subjects_df*
  * This means Subject_subjects has multiple rows for any subject that is modified after its creation

##### _Stage 2_

Produces __Feedbacks_df__ & __answers_df__

1. For every subject, get the most recent Feedback in ColumnStoreDB
2. Add Feedbacks & answers since the most recent Feedback in ColumnStoreDB (if the API returns it)
  * __answers_df__ is a combination of the Feedbacks & Initiates endpoints of Subject


##### _Stage 3_

Append DataFrames to ColumnStoreDB 

##### _Stage 4_

Append DataFrames to Hive 
